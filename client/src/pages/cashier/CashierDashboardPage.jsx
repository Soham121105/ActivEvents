...
// NEW STYLES
const IdentifierToggle = styled.div`
  display: grid;
...
const ActionBtn = styled.button`
  ...
  transition: all 0.2s;
  &:hover:not(:disabled) {
    background: ${p => p.primary ? '#4f46e5' : (p.danger ? '#fecaca' : '#e2e8f0')};
  }
...
export default function CashierDashboardPage() {
  ...
  const handleCheckBalance = async () => {
    if (!identifier) {
      setModal({ type: 'error', title: 'Error', text: 'Please enter an identifier first.' });
      return;
    }
    setLoading(true);
    try {
      const res = await cashierAxios.post('/cashier/check-balance', {
        identifier,
        identifierType,
      });
      setModal({
      ...
      setModal({
        type: 'error',
        title: 'Error',
        text: err.response?.data?.error || 'Could not check balance',
      });
    }
    setLoading(false);
  };
  ...
  const handleTopUp = async () => {
    ...
    if (!identifier) {
      setModal({ type: 'error', title: 'Error', text: 'Please enter a phone number or membership ID.' });
      return;
    }
    if (!amount || isNaN(topupAmount) || topupAmount <= 0) {
      setModal({ type: 'error', title: 'Error', text: 'Please enter a valid positive amount to top-up.' });
      return;
    }
    ...
    try {
      ...
      setModal({
        ...
        text: `${res.data.name}'s new balance is ₹${res.data.new_balance}.`,
      });
      clearForm();
      ...
      setModal({
        type: 'error',
        title: 'Top-Up Failed',
        text: err.response?.data?.error || 'Error processing top-up',
      });
    }
    setLoading(false);
  };
  ...
  const handleRefund = async () => {
    ...
    if (!window.confirm(
      `Are you sure you want to refund the ENTIRE remaining balance for ${identifier}? This action cannot be undone.`
    )) {
      return;
    }
    ...
    setLoading(true);
    try {
      const res = await cashierAxios.post('/cashier/refund', {
        identifier,
        identifierType,
      });
      const memberMsg = res.data.isMember
        ? 'This amount will be credited to their membership card.'
        : '';
      setModal({
        type: 'success',
      ...
      setModal({
        type: 'error',
        title: 'Refund Failed',
        text: err.response?.data?.error || 'Error processing refund',
      });
    }
    setLoading(false);
  };
  ...
  return (
    <PageContainer>
      <Header>
        <BrandInfo>
          <ClubName>{cashier?.club_name}</ClubName>
        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
          <Link
            to="/cashier/log"
            style={{ fontWeight: 600, color: '#6366f1', textDecoration: 'none' }}
          >
            View Full Log
          </Link>
          <LogoutBtn
            onClick={() => {
              logout();
              navigate(`/${cashier?.url_slug}/cashier-login`);
            }}
          >
            Logout
          </LogoutBtn>
          ...
          <WidgetBody>
            <InputGroup>
              ...
              <IdentifierToggle>
                ...
                <ToggleBtn
                  active={identifierType === 'member'}
                  onClick={() => {
                    setIdentifierType('member');
                    setIdentifier('');
                  }}
                >
                  Membership ID
            ...
            <InputGroup>
              <Label htmlFor="identifier">
                {identifierType === 'phone'
                  ...
                  : 'Visitor Membership ID'}
              </Label>
              <Input
                id="identifier"
                type={identifierType === 'phone' ? 'tel' : 'text'}
                placeholder={
                  identifierType === 'phone' ? '9876543210' : 'MEMBER-123'
                }
                value={identifier}
                onChange={(e) => {
            ...
            <ButtonGroup>
              <ActionBtn
                onClick={handleTopUp}
                primary
                disabled={loading || !identifier || !amount}
              >
                Top-Up Wallet
              </ActionBtn>
              <ActionBtn
                ...
                <LogItem key={l.cash_ledger_id}>
                  <div>
                    <div
                      style={{
                        color:
                          l.transaction_type === 'TOPUP'
                            ? '#059669'
                            : '#dc2626',
                        textTransform: 'capitalize',
                      }}
                    >
                      {l.transaction_type.toLowerCase()}
                    </div>
      ...
      {modal && (
        <ModalBG onClick={() => setModal(null)}>
          <ModalCard onClick={(e) => e.stopPropagation()}>
            ...
            <ActionBtn onClick={() => setModal(null)} style={{ background: '#0f172a', color: 'white' }}>
              Done
            </ActionBtn>
          </ModalCard>
        </ModalBG>
