...
export default function StallTransactionsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // This API call is secured by authMiddleware.js
        const res = await axios.get('http://localhost:3001/api/stalls/transactions');
        setData(res.data);
      } catch (err) {
        console.error("Error fetching transactions:", err)
        setError('Failed to fetch transactions. You may be logged out.');
      } finally {
        setLoading(false);
      }
    };
