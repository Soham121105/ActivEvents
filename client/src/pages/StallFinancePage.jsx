...
const PageSubheader = styled.h2`
  font-size: 1.25rem; font-weight: 500; color: #6b7280;
...
const StatValue = styled.p`
  ...
  color: #111827; margin: 8px 0;
`;
const HeroStatCard = styled(StatCard)`
  background-color: #eef2ff;
  ${StatValue} { color: #4338ca; }
`;
...
const LogAmount = styled.span`
  font-weight: 600; font-size: 1rem; text-align: right;
...
export default function StallFinancePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`http://localhost:3001/api/events/${eventId}/stalls/${stallId}/transactions`);
        setData(res.data);
      } catch (err) {
  ...
  return (
    <>
      ...
      <PageSubheader>
        Commission Rate: {(summary.commission_rate * 100 || 0).toFixed(0)}%
      </PageSubheader>
