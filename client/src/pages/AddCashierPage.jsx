...
// --- New Modal Styling (for the generated PIN) ---
...
const PinDisplay = styled.div`
  ...
  border-radius: 8px;
  padding: 16px;
  font-size: 2.25rem; /* Make PIN large */
  font-weight: 700;
  letter-spacing: 0.1em; /* Space out the numbers */
  color: #7c3aed; /* Purple */
  margin: 16px 0;
`;
// --- End of Styling ---
...
export default function AddCashierPage() {
  ...
  const handleAddCashier = async (e) => {
    ...
    try {
      // Call our new Event-Centric API
      const response = await axios.post(`http://localhost:3001/api/events/${eventId}/cashiers`, {
        cashier_name: cashierName,
      });

      // --- SUCCESS! ---
      // This is the new feature: show the modal with the PIN
      setNewCashier(response.data); // Save the new cashier data
  ...
  const closeAndRedirect = () => {
    setShowModal(false);
    // Redirect back to the event hub
  ...
  return (
    <>
      ...
      {/* --- This is the new "PIN Generated" Modal --- */}
      {showModal && (
        <ModalBackdrop>
          <ModalContent>
            ...
            <InputGroup style={{textAlign: 'left'}}>
              ...
              <PinDisplay style={{fontSize: '1.25rem', color: '#1f2937'}}>
                {newCashier.cashier_name}
              </PinDisplay>
            </InputGroup>
            <InputGroup style={{textAlign: 'left'}}>
